    ...
    if (!admin.apps.length) { // Check if any Firebase apps have been initialized
      this.firebaseApp = admin.initializeApp({
        credential: admin.credential.cert({
          ...
          privateKey: process.env.FIREBASE_PRIVATE_KEY.replace(/\\n/g, '\n'), // Ensure newlines are correctly processed
  ...
  async sendPushNotification(token: string, title: string, body: string) {
    ...
    try {
      ...
      if (error.errorInfo && error.errorInfo.code === 'messaging/registration-token-not-registered') {
        console.log('Token not registered. Consider updating or removing this token.');
        // Logic to handle or remove the invalid token
      }
    }
  }
